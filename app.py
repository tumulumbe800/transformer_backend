...
# ML features — oil level excluded
ML_FEATURES = ["oil_temp", "winding_temp", "current", "vibration"]

# Oil level hard thresholds
OIL_NORMAL   = 60.0   # above this = normal
OIL_WARNING  = 40.0   # below this = warning, above OIL_CRITICAL = warning
OIL_CRITICAL = 40.0   # below this = critical

# Anomaly score threshold
ANOMALY_THRESHOLD = 0.15  # tune after seeing real scores
...
def predict_anomaly(oil_temp, winding_temp, current, vibration):
    """Returns (anomaly_score, is_anomaly, ml_severity, fault_type)"""
    ...
    # Fault type from dominant feature deviation
    fault_type = "NORMAL"
    if is_anomaly:
        X_mean     = scaler.mean_
...
def oil_severity(oil_level):
    """Hard threshold check for oil level"""
...
def log_reading():
    ...
    # Insert reading
    cur = db.execute("""
        INSERT INTO readings (timestamp, oil_temp, winding_temp, current, vibration, oil_level)
        VALUES (?, ?, ?, ?, ?, ?)
    ...
    oil_sev, oil_fault = oil_severity(oil_level)

    # Combine severities — worst wins
...
def api_train():
    """
    Receives trained model files from Colab as base64.
    Saves to disk and reloads into memory.
    """
    import base64
    global model, scaler
    ...
    try:
        ...
        db = get_db()
        db.execute("""
            INSERT INTO model_meta (trained_at, n_samples, model_version, contamination)
            VALUES (?, ?, ?, ?)
...
<head>
  ...
  <title>Transformer PM Dashboard</title>
  <style>
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .status {{ background: #161b22; border: 2px solid {color};
...
<body>
  ...
  <div class="status">
    <h2>{'🔴 CRITICAL — ' if sev==2 else '🟡 WARNING — ' if sev==1 else '🟢 NORMAL — '}
    {last.get('fault_type','—')}</h2>
    <p>Health Index: {last.get('health_index', 100):.1f}% &nbsp;|&nbsp;
       Anomaly Score: {last.get('anomaly_score', 0):.4f} &nbsp;|&nbsp;
       Total Readings: {n_total} &nbsp;|&nbsp; Anomalies: {n_anomaly}</p>
  </div>

  <div class="grid">
    <div class="card"><h3>OIL TEMP</h3>
      <div class="val">{last.get('oil_temp','—')} °C</div></div>
    <div class="card"><h3>WINDING TEMP</h3>
      <div class="val">{last.get('winding_temp','—')} °C</div></div>
    <div class="card"><h3>CURRENT</h3>
      <div class="val">{last.get('current','—')} A</div></div>
    <div class="card"><h3>VIBRATION</h3>
      <div class="val">{last.get('vibration','—')} m/s²</div></div>
    <div class="card"><h3>OIL LEVEL</h3>
      <div class="val">{last.get('oil_level','—')} %</div></div>
    <div class="card"><h3>MODEL</h3>
      <div class="val" style="font-size:1em">
        {'✅ READY' if model else '⏳ TRAINING'}</div></div>
  </div>
  ...
  <table>
    <tr>
      ...
      <th>SCORE</th><th>FAULT</th><th>SEV</th>
    </tr>
    {''.join(f"""<tr>
      <td>{r.get('timestamp','')[-8:]}</td>
      <td>{r.get('oil_temp','')}</td>
      <td>{r.get('winding_temp','')}</td>
      <td>{r.get('current','')}</td>
      <td>{r.get('vibration','')}</td>
      <td>{r.get('oil_level','')}</td>
      <td>{r.get('anomaly_score',0):.4f if r.get('anomaly_score') else '—'}</td>
      <td>{r.get('fault_type','—')}</td>
      <td class="s{r.get('alert_severity',0)}">{r.get('alert_severity','—')}</td>
    </tr>""" for r in rows)}
  </table>
  <p style="color:#8b949e; margin-top:10px; font-size:0.75em">
    Auto-refresh every 5s &nbsp;|&nbsp; UTC time
  </p>
</body>
</html>"""
    return html
